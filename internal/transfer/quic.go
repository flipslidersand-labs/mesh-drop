...
func Send(ctx context.Context, addr, filePath string, nChunks int, fingerprint []byte, compressed bool, compLevel int) error {
	conn, err := quic.DialAddr(ctx, addr, clientTLSForFingerprint(fingerprint), quicConfig())
	if err != nil {
		return fmt.Errorf("dial %s: %w", addr, err)
	}
	return doSend(ctx, conn, filePath, nChunks, compressed, compLevel)
}

// SendNAT は NAT Traversal 済みソケット経由でファイルを送信する。
// #160: fingerprint is the SHA-256 of the receiver's TLS certificate DER.
func SendNAT(ctx context.Context, udpConn *net.UDPConn, peerAddr *net.UDPAddr, filePath string, nChunks int, fingerprint []byte, compressed bool, compLevel int) error {
...
func doSend(ctx context.Context, conn *quic.Conn, filePath string, nChunks int, compressed bool, compLevel int) error {
	...
	for i := 0; i < nChunks; i++ {
		...
		go func(i int) {
			...
			errs[i] = sendChunk(ctx, conn, f, i, offset, size, bar, peerKey, compressed, compLevel)
		}(i)
	}
	wg.Wait()
	fmt.Println()
...
func sendChunk(ctx context.Context, conn *quic.Conn, f *os.File, index int, offset, size int64, bar io.Writer, peerKey []byte, compressed bool, compLevel int) error {
	stream, err := conn.OpenStreamSync(ctx)
	if err != nil {
	...
	if compressed {
		...
		// bar は非圧縮バイト数で進捗を示す（ユーザーにとって自然）。
		_, err = io.Copy(enc, io.TeeReader(sr, bar))
